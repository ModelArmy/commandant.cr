...
### How rulesets are produced

```mermaid
flowchart LR
    A[Tool name + risk schema] --> B[LLM generation]
    B --> C[Human review]
    C --> D[Committed ruleset JSON]
    D --> E[commandant-rules-core]
```

### How commands are assessed

The library is organised into three layers — Platform/Parser (tokenisation), Ruleset (rule loading and matching), and Assessment (orchestration and response) — that connect as follows:
...
---
graph TD
    A[Platform] -->|selects parser + ruleset folder| B[Parser]
    B -->|produces| C[ParsedCommand]
    C --> D[Evaluator]
    E[RulesetStore] -->|loads Ruleset| D
    D -->|matched rules + risk tags| F[Assessor]
    G[ConstraintChecker] -->|violations| F
    H[PersistenceTracker] -->|signal| F
    F -->|produces| I[AssessmentResponse]
```

The step-by-step pipeline below shows how the Assessor orchestrates these components for each command invocation.
...
**1. Parse** — the platform's default parser tokenises the raw string into a `ParsedCommand` with `binary`, `flags`, `arguments`, `compounds`, and `subshells`. On POSIX platforms this is `PosixParser`; on Windows it is `CmdParser` or `PowerShellParser`. If the string is empty or whitespace-only, an empty `ParsedCommand` is returned immediately.
...
The platform determines both the parser and the ruleset folder searched:

|Platform                       |Parser            |Ruleset folders searched|
|-------------------------------|------------------|------------------------|
|`Platform::Linux`              |`PosixParser`     |`linux/`, then `posix/` |
|`Platform::MacOS`              |`PosixParser`     |`macos/`, then `posix/` |
|`Platform::Windows::Cmd`       |`CmdParser`       |`windows/` only         |
|`Platform::Windows::PowerShell`|`PowerShellParser`|`windows/` only         |
...
2. **`matched_rules`** — which rules fired? If the expected rule didn't fire, the `MatchSpec` didn't match. Check `flag_canonicals` against `flags_any`/`flags_all` in the rule. Remember values are exact strings including the prefix (`-r`, not `r`). On Windows (cmd.exe), flag canonicals are **uppercased** — `/s` in the command becomes `/S` in the canonical, so rulesets must use `/S` in `flags_any`.
